...
class CWSGENERATOR:
    ...
    def __init__(self,
        ...
        if out_mpp is None:
            ...
            print([cws_objective_value, self.objective_power, out_mpp_target_objective, in_mpp, out_mpp])
            self.cws_objective_value = cws_objective_value*(self.objective_power/out_mpp_target_objective)*(in_mpp/out_mpp)
    ...
    def slide_thumbnail(self):
        ...
        if file_type == '.tif' or file_type == '.tiff':
            ...
            slide_dimension_ss1 = (slide_dimension_rescale / 16).astype(np.int)
            cws_read_size = np.array(self.cws_read_size)
            cws_read_size_ss1 = np.round(cws_read_size/16).astype(np.int)
            slide_dimension_thumb = np.round(np.multiply(slide_dimension_rescale, np.divide(cws_read_size_ss1, cws_read_size))).astype(np.int)
    ...
    def create_thumb_from_tiles(self, thumb_size, out_tile_size):
        ...
        for h in range(int(math.ceil((thumb_size[1] - out_tile_size[1]) / out_tile_size[1] + 1))):
            for w in range(int(math.ceil((thumb_size[0] - out_tile_size[0]) / out_tile_size[0] + 1))):
                ...
                im = Image.open(os.path.join(tile_dir, 'Da' + str(iter_tot) + '.jpg'))
