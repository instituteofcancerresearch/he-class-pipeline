...
do
  ...
  jabname="HEDr"  
  sbatch --error=$var1 --output=$var2 "$pipe_path/old_sbatch_single.sh" $image_file $base_image_file $code_path $steps $conda_dir $tilePath $segmentationTilePath $cellDetectionResultsPath $cellClassificationResultsPath
  ((counter++))
done
