...
function [im_rot, csv_table_rot] = rotate_annotations(im, csv_table, angle)
    ...
    csv_table_rot = table(csv_table.V1, round((csv_table.V2-(im_size(2)/2)).*cosd(angle)+(csv_table.V3-(im_size(1)/2)).*sind(angle)+rot_offset(2)), round((csv_table.V3-(im_size(1)/2)).*cosd(angle)-(csv_table.V2-(im_size(2)/2)).*sind(angle)+rot_offset(1)), 'VariableNames', {'V1', 'V2', 'V3'});
